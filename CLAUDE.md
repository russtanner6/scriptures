...
- **Gemini 10-Output Pipeline:** All chapter-level data is being rebuilt from scratch via Gemini. Guide at `~/Desktop/GEMINI-SCORING-GUIDE.md`. Process: user feeds book text to Gemini → Gemini returns 10 JSON arrays → user pastes to Claude Code → Claude Code enriches and appends. **After every book:** run completeness audit, spot-check accuracy, verify speakers against characters.json, add any truly missing characters, update docs. Progress tracked in SESSION-LOG.md. **Completed: BOOK OF MORMON (all 15 books) + PEARL OF GREAT PRICE (all 5 books) = 255 chapters.** Now starting Apocrypha, then OT, NT, D&C. ~1,509 chapters remaining across 82 books.
- **Character data:** `data/characters.json` — 761 named individuals with bios, aliases, family relationships, portraits (~40 of 71 prominent), volumes, tiers (1-4). Count grows as Gemini pipeline identifies missing people. Tiers 1-2 are "prominent" (71 people). `/api/chapter-characters` finds characters per chapter via speaker matching + whole-word text scanning with volume-aware deduplication.
