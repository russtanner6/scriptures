...
- **Context Eggs system:** `data/context-eggs.json` → `/api/context-eggs` → ScriptureReader. ~930+ scholarly insights with 5 categories (Linguistic, Historical, Cultural, Literary, Restoration). Domain isolation: Restoration-category eggs hidden when LDS volumes toggled off. EggMarker pills (4px borderRadius, amber/gold) display below verse text alongside ResourceMarker pills. EggPopover shows parchment/slate card with title, insight, source citation. Keywords get subtle glint animation (7s stagger per egg). "Context" toggle in reader layers section.
